...
class ShareViewController: UIViewController {
        ...
        attachment.loadItem(forTypeIdentifier: UTType.image.identifier, options: nil) { [weak self] item, error in
            ...
            if let imageURL = item as? URL {
                do {
                    ...
                    self?.completeRequest(error: "画像ファイルの読み込みに失敗しました: \(error.localizedDescription)")
                }
                return
            }
    ...
    private func saveSharedFileAndOpenApp(fileData: Data, fileName: String) {
        do {
            // アプリグループの共有ディレクトリを取得
            guard let sharedContainerURL = FileManager.default.containerURL(forSecurityApplicationGroupIdentifier: "group.com.meikenn.tama") else {
                completeRequest(error: "共有コンテナにアクセスできません")
                return
            }
            
            // 一時保存用ディレクトリを確保
            let tempDirURL = sharedContainerURL.appendingPathComponent("PrintSharedFiles", isDirectory: true)
