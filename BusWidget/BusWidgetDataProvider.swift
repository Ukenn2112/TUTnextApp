...
struct BusWidgetDataProvider {
    ...
    static func getNextBusTimes(routeType: String, scheduleType: String, from date: Date) -> [BusWidgetSchedule.TimeEntry] {
        ...
        for busTimeData in busTimesData {
            if let hour = busTimeData["hour"] as? Int,
               let minute = busTimeData["minute"] as? Int {
                ...
                times.append(BusWidgetSchedule.TimeEntry(hour: hour, minute: minute, isSpecial: isSpecial, specialNote: specialNote))
            }
        }
