...
class PrintSystemService {
    ...
    func login(completion: @escaping (Bool, Error?) -> Void) {
        guard let url = URL(string: "\(baseURL)/guestweb/login") else {
            completion(false, NSError(domain: "PrintSystemService", code: 0, userInfo: [NSLocalizedDescriptionKey: "Invalid URL"]))
            return
        }
        ...
        APIService.shared.request(request: request, logTag: "印刷システムログイン") { data, response, error in
            ...
            if let httpResponse = response as? HTTPURLResponse {
                ...
                completion(false, NSError(domain: "PrintSystemService", code: 1, userInfo: [NSLocalizedDescriptionKey: "Invalid response"]))
            }
        }
    }
    
    // ファイルをアップロードする
    func uploadFile(fileData: Data, fileName: String, settings: PrintSettings, completion: @escaping (PrintResult?, Error?) -> Void) {
        guard let url = URL(string: "\(baseURL)/api/tenants/2102%3ACOD1/user/prints/") else {
            completion(nil, NSError(domain: "PrintSystemService", code: 0, userInfo: [NSLocalizedDescriptionKey: "Invalid URL"]))
            return
        }
        ...
        body.append("--\(boundary)\r\n".data(using: .utf8)!)
        body.append("Content-Disposition: form-data; name=\"startPage\"\r\n\r\n".data(using: .utf8)!)
        ...
        // autoNetprintを追加
        body.append("--\(boundary)\r\n".data(using: .utf8)!)
        body.append("Content-Disposition: form-data; name=\"autoNetprint\"\r\n\r\n".data(using: .utf8)!)
        body.append("false\r\n".data(using: .utf8)!)
        ...
        // リクエストを実行
        APIService.shared.request(request: request, logTag: "印刷ファイルアップロード") { data, response, error in
            if let error = error {
            ...
            guard let data = data else {
                completion(nil, NSError(domain: "PrintSystemService", code: 2, userInfo: [NSLocalizedDescriptionKey: "No data received"]))
                return
            }
            ...
            do {
                ...
                if let json = try JSONSerialization.jsonObject(with: data) as? [String: Any],
                   let id = json["id"] as? String {
                    ...
                    completion(nil, NSError(domain: "PrintSystemService", code: 3, userInfo: [NSLocalizedDescriptionKey: "Invalid JSON format"]))
                }
            } catch {
    ...
    // 印刷詳細情報を取得する
    private func fetchPrintDetails(id: String, retryCount: Int = 0, completion: @escaping (PrintResult?, Error?) -> Void) {
        ...
        APIService.shared.request(request: request, logTag: "印刷詳細情報取得") { [weak self] data, response, error in
                    ...
                    if let prCode = json["prCode"] as? String, !prCode.isEmpty {
                        ...
                        if let expiresAt = json["expiresAt"] as? String,
                           let utcDate = dateFormatter.date(from: expiresAt) {
                            ...
                            completion(nil, NSError(domain: "PrintSystemService", code: 5, userInfo: [NSLocalizedDescriptionKey: "Invalid date format"]))
                        }
                    } else {
                        // 50回まで再試行
                        if retryCount < 50 {
                            ...
                            DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
                                self.fetchPrintDetails(id: id, retryCount: retryCount + 1, completion: completion)
                            }
                        } else {
                            print("prCodeの取得に失敗しました。50回の再試行後も取得できませんでした。")
                            completion(nil, NSError(domain: "PrintSystemService", code: 4, userInfo: [NSLocalizedDescriptionKey: "Failed to get prCode after 50 retries"]))
                        }
                    }
                } else {
                    completion(nil, NSError(domain: "PrintSystemService", code: 3, userInfo: [NSLocalizedDescriptionKey: "Invalid JSON format"]))
