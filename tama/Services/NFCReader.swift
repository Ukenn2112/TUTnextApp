...
class NFCReader: NSObject, ObservableObject, NFCTagReaderSessionDelegate {
    ...
    func tagReaderSession(_ session: NFCTagReaderSession, didInvalidateWithError error: Error) {
        ...
        // NFCError.Domainグローバル定数を使用してドメインを確認し、
        // エラーコードで直接切り替える
        ...
        Task {
            do {
                ...
                guard currentSystemCode == self.felicaSystemCode else {
                    session.invalidate(errorMessage: "対応していないカードです (システムコード: \(currentSystemCode.hexEncodedString()))。")
                    print("システムコードの不一致。期待: \(self.felicaSystemCode.hexEncodedString()), 取得: \(currentSystemCode.hexEncodedString())")
                    return
                }
    ...
    private func readFeliCaBlocks(
        ...
        guard status1 == 0x00, status2 == 0x00 else {
            ...
            throw NFCReadError.readFailed(serviceCode: serviceCode.hexEncodedString(), blockIndex: -1)
        }
    ...
    private func processFeliCaData(studentInfo: [Data], personalInfo: [Data]) throws {
        ...
        let fullStudentID = (idPart1.trimmingCharacters(in: .whitespacesAndNewlines) +
        ...
        if let validityString = studentInfo[1].string(encoding: .ascii)?.trimmingCharacters(in: .whitespacesAndNewlines) {
            let startDate = String(validityString.prefix(8))
            let endDate = String(validityString.suffix(8))
        ...
        let cleanLastNameTemp = lastName
            .components(separatedBy: unwantedCharacters)
            .joined()
            .trimmingCharacters(in: unwantedCharacters)

        let cleanFirstNameTemp = firstName
            .components(separatedBy: unwantedCharacters)
            .joined()
            .trimmingCharacters(in: unwantedCharacters)

        let cleanLastName = cleanLastNameTemp.applyingTransform(.fullwidthToHalfwidth, reverse: true) ?? cleanLastNameTemp
        let cleanFirstName = cleanFirstNameTemp.applyingTransform(.fullwidthToHalfwidth, reverse: true) ?? cleanFirstNameTemp
